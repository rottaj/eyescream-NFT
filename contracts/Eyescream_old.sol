...
contract Eyescream is Ownable, ERC721URIStorage {
    ...
    function mint(string[] calldata _svgs) external payable {
        require(_tokenCounter.current() < MAX_SUPPLY, "SOLD OUT");
        require(_tokenCounter.current() + _svgs.length <= MAX_SUPPLY, "SOLD OUT");
        require(_svgs.length > 0, "AMOUNT CANNOT BE ZERO");
        require(_svgs.length <= MAX_PER_TX, "AMOUNT EXCEEDED PER TXN");
        require(msg.value == (_svgs.length * PRICE), "PRICE LIMIT NOT REACHED");
        for (uint256 i=0; i<=_svgs.length; i++) {
            //string memory svg = string(abi.encodePacked(_svgs[i])); //convert bytes32 to string
            string memory _imgURI = svgToURI(_svgs[i]);
